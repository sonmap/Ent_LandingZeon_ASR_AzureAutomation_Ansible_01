...
```bash
DRY_RUN=true ./30-actual-failover-guarded.sh
DRY_RUN=false ./30-actual-failover-guarded.sh
```
...
- 실제 Failover / Traffic Switch 스크립트는 `DRY_RUN=true`가 기본입니다.
- 실제 실행 전 `APPROVAL_TICKET` 값을 요구합니다.
- 실제 실행 전 승인 문구 입력을 요구합니다.
- ASR 실제 명령은 환경별 Recovery Plan/Protected Item 확인 후 사용하도록 주석과 TODO를 포함합니다.
